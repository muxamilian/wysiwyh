...
On the top left you see the original video. On the bottom there's the audio spectrum, which is the output of the *recursively structured* encoder.  The audio spectrum goes from 1 Hz on the left to 5000 Hz on the right. The unit on the y axis is arbitrary. On the top right is the video reconstructed by the decoder using the audio spectrum of the encoder. 
